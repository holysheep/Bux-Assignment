...
internal data class RealtimeUpdateEvent(
    @Json(name = "t") val type: UpdateEventType,
    val body: UpdateEventBody
)

@JsonClass(generateAdapter = true)
internal data class UpdateEventBody(
    @Json(name = "securityId") val productId: String?,
    @Json(name = "currentPrice") val updatedPrice: BigDecimal?
)

internal enum class UpdateEventType(val value: String) {
